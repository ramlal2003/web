...
def index():
    if request.method == 'POST':
        ...
        if 'file' in request.files:
            ...
            if file and allowed_file(file.filename):
                try:
                    ...
                    filename = secure_filename(f"temp_{session_id}_{timestamp}_{file.filename}")
                    file_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
                    file.save(file_path)
                    session['current_file'] = file_path
                    
                    # Generate unique output SVG filename
                    output_filename = f"temp_{session_id}_{timestamp}_output.svg"
                    output_path = os.path.join(app.config['STATIC_FOLDER'], output_filename)
                    session['current_svg'] = output_filename
                    ...
                    return jsonify({'success': True, 'svg_path': f'/static/{output_filename}'})
                except Exception as e:
        ...
        if request.form.get('update'):
            if 'current_file' not in session or 'current_svg' not in session:
            ...
            try:
                ...
                output_path = os.path.join(app.config['STATIC_FOLDER'], session['current_svg'])
                ...
                return jsonify({'success': True, 'svg_path': f'/static/{session["current_svg"]}'})
            except Exception as e:
...
def download_png():
    if 'current_svg' not in session:
        return 'No SVG file found', 404
        
    svg_path = os.path.join(app.config['STATIC_FOLDER'], session['current_svg'])
